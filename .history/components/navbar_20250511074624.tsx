const handleSubmit = async(e : React.FormEvent) => {
    ...
    try {
        ...
        toast({title: "Processing...", description: "Uploading product data to IPFS..."});
        ...
        toast({title: "IPFS Upload Complete", description: "Product data successfully stored on IPFS"});

        // 2. Mint NFT with IPFS link as metadata
        toast({title: "Processing...", description: "Minting NFT on the blockchain..."});

        const tokenName = `${formData
            .name
            .replace(/\s+/g, '_')}_${Date
            .now()}`;

        // THAY THAY ĐỔI Ở ĐÂY: Lấy địa chỉ đúng cách từ Mesh SDK Sử dụng hàm
        // getUsedAddresses() hoặc getRewardAddresses() thay vì getChangeAddress()
        ...
        const mintResult = await mintNFT(wallet, tokenName, nftMetadata);
